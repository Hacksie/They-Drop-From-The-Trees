    ...
    public class Spear : MonoBehaviour
    {
        ...
        void OnTriggerEnter(Collider other)
        {
            if(other.CompareTag("Enemy"))
            {
                ...
                DamageNumbersPool.Instance.Spawn(transform.position, transform.forward, damage.ToString());                
                gameObject.SetActive(false);
            }
            else if(other.CompareTag("Terrain"))
            {
                gameObject.SetActive(false);
                DamageNumbersPool.Instance.Spawn(transform.position, transform.forward, "miss"); 
            }
        }
    }
}